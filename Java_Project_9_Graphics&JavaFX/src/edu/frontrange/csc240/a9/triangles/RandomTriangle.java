 ...
 * @version		2.1, 2017-10-27, CSC-240 Assignment 9
 */
 ...
 * @param backGroundColor	background color to be avoided
 */
public RandomTriangle(double width, double height, Color backGroundColor)
{
	...
	analyzeBackGroundColor(backGroundColor);
...
/**
 * Het the components of the background color. The random color chosen will not
 * be permitted to use values near these components.
 *
 * @param color		the color to analyze
 */
private void analyzeBackGroundColor(Color color)
{
	backGroundColor = new double[3];		// this "3" is inherent to Color
	backGroundColor[0] = color.getRed();
	backGroundColor[1] = color.getGreen();
	backGroundColor[2] = color.getBlue();
}
...
{
	final double[] rgb = new double[backGroundColor.length];
	...
	for( int h = 0; h != rgb.length; ++h )
	{
		...
		do
		{
			...
			double bottom_of_range = backGroundColor[h] - LIMIT / 2.0F;
			double top_of_range = backGroundColor[h] + LIMIT / 2.0F;
